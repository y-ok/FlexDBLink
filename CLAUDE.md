...
- テストコードで `setAccessible` やリフレクション（`getDeclaredField`、`getDeclaredConstructor`、`Class.forName` 等）を使用しないこと。
- 同一パッケージからの直接アクセスに置き換えること。private メソッドのテストは不要。public API 経由でカバレッジ100%を達成すること。
- public API 経由でカバーできない場合はプロダクションコードの構造を見直すこと。
